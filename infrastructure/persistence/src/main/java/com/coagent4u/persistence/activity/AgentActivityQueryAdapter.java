...
public class AgentActivityQueryAdapter implements AgentActivityQueryPort {
    ...
    @Override
    public List<AgentActivityEntry> findByUserId(UserId userId, String eventTypeFilter, int offset, int limit) {
        ...
        Page<AgentActivityJpaEntity> result;
        if (eventTypeFilter != null && !eventTypeFilter.isBlank()) {
            result = repository.findByAgentIdAndEventTypeOrderByOccurredAtDesc(
                    agentId, eventTypeFilter, pageRequest);
        } else {
            result = repository.findByAgentIdOrderByOccurredAtDesc(agentId, pageRequest);
        }
    ...
    public long countByUserId(UserId userId, String eventTypeFilter) {
        ...
        if (eventTypeFilter != null && !eventTypeFilter.isBlank()) {
            return repository.countByAgentIdAndEventType(agentId, eventTypeFilter);
        }
